...
async def add_game(bot: MyClient, message: nextcord.Message) -> None:
    ...
    if len(split) > 1:
        ...
        bot.cache.add_game_to_blacklist(wanted_game)
...
async def check_for_banned_games(bot: MyClient, message: nextcord.Message) -> None:
    ...
    bad_members = await search_guild(guild, bot.cache)
...
async def search_guild(guild: nextcord.Guild, cache: GameCache) -> list[nextcord.Member]:
    ...
    for member in flat_members:
        if next((game.name for game in member.activities if cache.game_in_blacklist(game.name)), None) and not cache.member_in_whitelist(member.id):
